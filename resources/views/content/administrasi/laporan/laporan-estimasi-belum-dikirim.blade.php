...
<!-- Page Scripts -->
@section('page-script')
  <script src="{{ asset('assets/js/laporan-estimasi-belum-dikirim.js') }}"></script>
@endsection
...
<div class="row">
  ...
  <div class="col-12">
    <div class="card">
      ...
      <div class="card-body">
        <form id="filterForm" class="form-control-validation" method="post" action="{{ url('laporan-estimasi-dikirim-list') }}" autocomplete="off">
          @csrf
          <div class="row mb-4">
            <label class="col-sm-2 col-form-label" for="filter-tanggal">Tanggal</label>
            <div class="col-sm-10">
              <input type="text" id="filter-tanggal" name="tanggal" class="form-control dt-date" value="{{ @$datafilter['tanggal'] }}" placeholder="Pilih Tanggal" />
            </div>
          </div>
    ...
    <div class="card">
      <div class="card-header d-flex align-items-center justify-content-between">
        <div class="card-title mb-0">
          <h5 class="m-0 me-2">Laporan Estimasi Belum Dikirim</h5>
        </div>
      ...
      <div class="card-datatable">
        <table class="datatables-estimasi-belum-dikirim table table-bordered table-responsive" data-title="{{ $title }}" data-add="{{ $isAdd }}" data-edit="{{ $isEdit }}" data-delete="{{ $isDel }}">
          <thead>
            <tr>
              <th>No</th>
              <th>No. SPK</th>
