  ...
  <div class="d-flex justify-content-between flex-row">
    <div class="mb-6">
      <div class="d-flex svg-illustration mb-6 gap-2">
        <span class="app-brand-text fw-bold">Laporan Mobil Turun Lapangan</span>
      </div>
  ...
  <div class="border-bottom-0 border-top-0 rounded">
    <table class="table m-0">
      <thead>
        <tr>
          <th>No</th>
          <th>Turun Lapangan</th>
          <th>No. Turun Lapangan</th>
          <th>No. SPK</th>
          <th>No. Polisi</th>
      ...
      <tbody>
        ...
        <tr>
          <td>{{ $no++ }}</td>
          <td>{{ date("d/m/Y", strtotime($row->tgl_turun_lapangan ))}}</td>
          <td>{{ $row->kode_turun_lapangan }}</td>
          <td>{{ $row->kode_spk }}</td>
          <td>{{ $row->no_polisi }}</td>
          <td>{{ $row->merek_tipe }}</td>
          <td>{{ $row->nama_pelanggan }}</td>
          <td>{{ $row->pemilik }}</td>
          <td>{{ date("d/m/Y", strtotime($row->tgl_rencana_selesai ))}}</td>
          <td>{{ $row->status }}</td>
        </tr>
        @endforeach
      </tbody>
