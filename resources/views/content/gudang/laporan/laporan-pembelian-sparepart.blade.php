...
@section('page-script')
  <script src="{{ asset('assets/js/laporan-pembelian-sparepart.js') }}"></script>
@endsection
...
<div class="row">
  ...
  <div class="col-12">
    <div class="card">
      ...
      <div class="card-body">
        <form id="filterForm" class="form-control-validation" method="post" action="{{ url('laporan-pembelian-sparepart-list') }}" autocomplete="off">
          @csrf
          <div class="row mb-4">
            <label class="col-sm-2 col-form-label" for="filter-supplier">Supplier</label>
            <div class="col-sm-10">
              <select id="filter-supplier" name="supplier" class="select2 form-select" data-allow-clear="true">
                <option value="">Pilih Supplier</option>
                @foreach($pemasok as $row)
                  <option value="{{ $row->kode_pemasok }}" @if(old('supplier', @$datafilter['supplier']) == $row->kode_pemasok) selected @endif>{{ $row->nama_pemasok }}</option>
                @endforeach
              </select>
            </div>
          </div>
          <div class="row mb-4">
            <label class="col-sm-2 col-form-label" for="filter-no-spk">No. SPK</label>
            <div class="col-sm-10">
              <input type="text" id="filter-no-spk" name="no_spk" class="form-control" value="{{ @$datafilter['no_spk'] }}" placeholder="No. SPK" />
            </div>
          </div>
          <div class="row mb-4">
    ...
    <div class="card">
      <div class="card-header d-flex align-items-center justify-content-between">
        <div class="card-title mb-0">
          <h5 class="m-0 me-2">Laporan Pembelian Sparepart</h5>
        </div>
      ...
      <div class="card-datatable">
        <table class="datatables-pembelian-sparepart table table-bordered table-responsive" data-title="{{ $title }}"
