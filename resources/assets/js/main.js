...
const SearchConfig = {
  ...
  placeholder: 'HO (Head Office)-Anugerah Makmur',
  classNames: {
...
function initializeAutocomplete() {
  ...
  const el = document.querySelector(SearchConfig.container);
  const ph = el?.dataset?.placeholder?.trim();
  if (ph) SearchConfig.placeholder = ph;
          ...
          <div class="p-5 p-lg-12">
            <div class="row g-4">
              ${Object.entries(data.suggestions || {}).map(
                ([section, items]) => html`
                  <div class="col-md-12 suggestion-section">
