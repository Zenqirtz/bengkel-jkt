...
document.addEventListener('DOMContentLoaded', function (e) {
  ...
  if (dt_basic_table) {
    ...
    dt_basic = new DataTable(dt_basic_table, {
      ...
      serverSide: true,
      ajax: {
        url: baseUrl + 'order-pembelian-list',
        data: function (d) {
      ...
      dt_detail = new DataTable(dt_detail_table, {
        ...
        ajax: {
          ...
          data: function (d) {
            ...
            d.kode_order = $('#add-nomor-order').val(); //selected_kode_order;
            d.tipe_barang = $('#add-tipe-barang').val(); //selected_tipe_barang;
            d.kode_permintaan = $('#add-nomor-permintaan').val(); //selected_kode_permintaan;
        ...
        columnDefs: [
          ...
          },
          {
            ...
            render: function (data, type, full, meta) {
              ...
              return `
                ...
                <input type="hidden" class="harga-estimasi-ref" value="${full.harga_estimasi ?? 0}" />
              `;
            }
          },
          {
          ...
          },
          {
            ...
            visible: true,
            render: function (data, type, full, meta) {
              if (selected_tipe == 'detail-order-batal') {
                let chked = full.cek == '2' ? 'checked' : '';
                return `<input type="checkbox" name="detail[${full.id}][cek]" class="dt-checkboxes-det form-check-input" value="1" ${chked}>`;
              } else {
                if (full.cek == '2') {
                  return '<span class="badge rounded-pill text-bg-info" text-capitalized> DIBATALKAN </span>';
                } else {
                  return (
                    '<div class="d-flex align-items-center gap-4">' +
    ...
    //           if (result.value) {
    //             // delete the data
    //             fetch(`${baseUrl}order-pembelian-list/${user_id}`, {
    //               method: 'DELETE',
    //               headers: {
    ...
    document.addEventListener('click', function (e) {
      if (e.target.closest('.edit-record')) {
        if (isEdit) {
          ...
          // get data
          fetch(`${baseUrl}order-pembelian-list/${user_id}/edit`)
          ...
          fetch(`${baseUrl}order-pembelian-list/${user_id}/edit`)
            ...
            .then(result => {
              ...
              if (status) {
                ...
                // document.getElementById('add-nomor-po').value = data.no_po;
                ...
                setSelectValue('#add-tipe-bayar', data.tipe_bayar, '');
                setSelectValue('#add-pemasok', data.kode_pemasok, '');
          ...
          fetch(`${baseUrl}order-pembelian-list/${user_id}/edit`)
            ...
            .then(result => {
              ...
              if (status && data.status_approve == '0') {
                ...
                // document.getElementById('kode_order').value = data.kode_order;
                // document.getElementById('kode_permintaan2').value = data.kode_permintaan;
            ...
            .then(result => {
              ...
              if (status) {
                if (data.status_approve == '1') {
                  ...
                  const printUrl = `${baseUrl}gudang/order-pembelian-cetak?` + queryString;
    ...
    const btnBatalSelected = document.querySelector('.batal-selected-order');
    if (btnBatalSelected) {
      btnBatalSelected.addEventListener('click', function () {
        ...
        if (!selectedCheckbox) {
          ...
          // reset seluruh form (termasuk select2)
          // clearFormData();
          $('#add-memo-batal').addClass('is-invalid');
              ...
              if (status) {
                if (data.status_approve == '0') {
                  Swal.fire({
                    icon: 'warning',
                    title: `Peringatan!`,
                    text: `Data PO belum di appprove`,
                    customClass: {
                      confirmButton: 'btn btn-success'
                    }
                  });
                ...
                document.getElementById('user_id').value = data.id;
                document.getElementById('status_approve').value = '2';

                document.getElementById('add-nomor-order').value = data.kode_order;
                document.getElementById('add-tanggal-order').value = data.tanggal;
                // document.getElementById('add-nomor-po').value = data.no_po;
                document.getElementById('add-memo').value = data.memo;
                document.getElementById('add-memo-batal').value = data.memo_batal;
                document.getElementById('add-ppn').value = data.ppn;
                ...
                $('#add-tanggal-order').prop('disabled', true);
                $('#add-tipe-barang').prop('disabled', true);
                $('#add-tipe-bayar').prop('disabled', true);
                $('#add-pemasok').prop('disabled', true);
                $('#add-memo').prop('disabled', true);
                $('.add-ppn').prop('disabled', true);

                // selected_kode_order = data.kode_order;
                // selected_kode_cabang = data.kode_cabang;
                selected_tipe = 'detail-order-batal';
      ...
      const fv = FormValidation.formValidation(addNewDataForm, {
        fields: {
          ...
          tipe_bayar: {
            validators: {
              notEmpty: {
                message: 'Silahkan Input Pembayaran'
              }
            }
          },
          kode_pemasok: {
            validators: {
              notEmpty: {
                message: 'Silahkan Input Nama Supplier'
              }
            }
          }
        },
        ...
        fetch(`${baseUrl}order-pembelian-list`, {
          method: 'POST',
          headers: {
            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
    ...
    document.addEventListener('click', function (e) {
      ...
      if (editDetBtn) {
        if (isEdit) {
          ...
          document.getElementById('add-harga-bahan').dataset.hargaEstimasi = rowData.harga_estimasi ?? '0';
        } else {
          // Hide offcanvas
          if (tipe == 'detail') {
            const offcanvasInstance = bootstrap.Offcanvas.getInstance(offCanvasFormDet1);
      ...
      if (deleteDetBtn) {
        const tipe = deleteDetBtn.dataset.tipe;
         if (isAdd || isEdit) {   // ⬅️ ganti dari isDelete jadi ini
        // if (isDelete) {
          if (tipe == 'detail') {
            // 1. Ambil instance DataTable
            const tableEl = document.querySelector('.datatables-detail');
        ...
        if (cekID) {
          ...
          rowToEdit
            .data({
              ...
              harga_estimasi: oldData.harga_estimasi ?? '0', // ← TAMBAHKAN INI
    ...
    if (cekSPKBtn) {
      cekSPKBtn.addEventListener('click', function () {
        ...
        selected_tipe = 'permintaan-baru-all';
    ...
    if (btnSpkSelected) {
      btnSpkSelected.addEventListener('click', function () {
        ...
        if (!selectedCheckbox) {
          ...
          // Pastikan route URL sesuai dengan konfigurasi route Anda
          const url = `${baseUrl}gudang/order-pembelian-cek-spk?` + queryString;
          ...
          fetch(url)
            ...
            .then(result => {
              ...
              if (status) {
                ...
                if (data.tipe_barang == 'S' || data.tipe_barang == 'T') {
                  selected_tipe = 'detail-permintaan-part';
                  dt_detail.column(2).visible(true);
                } else {
                  selected_tipe = 'detail-permintaan-bahan';
                  dt_detail.column(2).visible(false);
                }
    ...
    invoiceItemPriceList.forEach(function (invoiceItemPrice) {
      if (invoiceItemPrice) {
        invoiceItemPrice.addEventListener('input', event => {
          ...
          // ✅ VALIDASI HARGA vs ESTIMASI
          if (invoiceItemPrice.id === 'add-harga-bahan') {
            const hargaEstimasi = parseFloat((invoiceItemPrice.dataset.hargaEstimasi ?? '0').replace(/,/g, '')) || 0;
            const hargaInput = parseFloat(invoiceItemPrice.value.replace(/,/g, '')) || 0;

            if (hargaEstimasi > 0 && hargaInput > hargaEstimasi) {
              const hargaFmt = new Intl.NumberFormat('id-ID').format(hargaEstimasi);
              Swal.fire({
                icon: 'warning',
                title: 'Peringatan Harga!',
                html: `Harga yang diinput <b>${new Intl.NumberFormat('id-ID').format(hargaInput)}</b> melebihi harga estimasi <b>${hargaFmt}</b>.<br>Pastikan harga sudah benar.`,
  ...
  if (selectBahan.length) {
    ...
    selectBahan.on('change', function () {
      ...
      if (kodebahan.length > 0 && tipe != 'S') {
        ...
        const url = `${baseUrl}api/bahan?` + queryString;
        ...
        fetch(url)
          ...
          .then(result => {
            ...
            setSelectValue('#add-satuan', data.kode_satuan, '');
          })
          .catch(err => {
...
function fetchDashboardOrder() {
  // Fetch data
  fetch(`${baseUrl}order-pembelian-list?tipe=total-data`)
    .then(response => {
      if (!response.ok) throw new Error('Gagal mengambil data');
      return response.json();
    })
...
function clearFormData() {
  ...
  $('#add-tanggal-order').addClass('is-invalid');
  $('#add-tipe-barang').addClass('is-invalid');
  $('#add-tipe-bayar').addClass('is-invalid');
  $('#add-pemasok').addClass('is-invalid');
