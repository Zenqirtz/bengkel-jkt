...
document.addEventListener('DOMContentLoaded', function (e) {
  ...
  if (dt_basic_table) {
      ...
      if (e.target.closest('.delete-record')) {
        if (isDelete) {
          ...
          Swal.fire({
            ...
            if (result.value) {
              // delete the data
              fetch(`${baseUrl}estimasi-list/${user_id}`, {
                method: 'DELETE',
                headers: {
    ...
    document.addEventListener('click', function (e) {
      if (e.target.closest('.edit-record')) {
        if (isEdit) {
          ...
          // get data
          fetch(`${baseUrl}estimasi-list/${user_id}/edit`)
        ...
        if (!selectedCheckbox) {
          ...
          .then(result => {
            ...
            if (status) {
              ...
              document.getElementById('add-disc-perbaikan').value = data.disc_perbaikan;
              document.getElementById('add-disc-sparepart').value = data.disc_sparepart;
              document.getElementById('add-disc-lain').value = data.disc_lain;
    ...
    const btnViewSelected = document.querySelector('.view-selected-spk');
    if (btnViewSelected) {
      btnViewSelected.addEventListener('click', function () {
        ...
        if (!isList) {
          Swal.fire({
            icon: 'error',
            title: 'Akses Ditolak',
            text: 'Anda tidak memiliki izin untuk melihat data.',
            customClass: { confirmButton: 'btn btn-primary' }
          });
          return;
        }
    ...
    const btnEditSelected2 = document.querySelector('.cetak-estimasi');
    if (btnEditSelected2) {
          ...
          fetch(`${baseUrl}estimasi-list/${user_id}`)
            ...
            .then(result => {
              ...
              if (status) {
                ...
                const printUrl = `${baseUrl}administrasi/cetak-estimasi?` + queryString;
    ...
    // EXPORT EXCEL
    const btnExportEstimasi = document.querySelector('.export-estimasi');
    if (btnExportEstimasi) {
      btnExportEstimasi.addEventListener('click', function () {
        const selectedCheckbox = document.querySelector('.datatables-spk .dt-checkboxes:checked');
        if (!selectedCheckbox) {
          Swal.fire({
            icon: 'warning',
        ...
        fetch(`${baseUrl}estimasi-list/${user_id}`)
          ...
          .then(({ status, message, data }) => {
            if (status) {
              window.location.href = `${baseUrl}administrasi/export-estimasi?id=${data.id}`;
            } else {
              Swal.fire({
                icon: 'warning',
                title: 'Peringatan!',
...
function hitungTotalPerbaikan() {
  ...
  let formattedPpnPerbaikan = new Intl.NumberFormat('en-US', {
    minimumFractionDigits: 2,
    maximumFractionDigits: 2
  }).format(ppn);

  let formattedPpnPart = new Intl.NumberFormat('en-US', {
    minimumFractionDigits: 2,
    maximumFractionDigits: 2
  }).format(ppn_part);

  let formattedPpnLain = new Intl.NumberFormat('en-US', {
    minimumFractionDigits: 2,
    maximumFractionDigits: 2
  }).format(ppn_lain);
  ...
  $('#ppn-perbaikan').text(formattedPpnPerbaikan);
  $('#ppn-sparepart').text(formattedPpnPart);
  $('#ppn-lain').text(formattedPpnLain);
...
//   // Ambil elemen input di baris tersebut
//   let elDisc1 = $('#add-disc-perbaikan');
//   let elDisc2 = $('#add-disc-sparepart');
//   let elDisc3 = $('#add-disc-lain');

//   // Ambil Nilai & Bersihkan format (hapus koma)
//   let disc1 = parseFloat(elDisc1.val().replace(/,/g, '')) || 0;
//   let disc2 = parseFloat(elDisc2.val().replace(/,/g, '')) || 0;
//   let disc3 = parseFloat(elDisc3.val().replace(/,/g, '')) || 0;

//   // Hitung Subtotal Baris
//   let subtotal = disc1 + disc2 + disc3;
...
//   // Update Kolom JUMLAH di baris tersebut
//   $('#disp-total-diskon').text(formattedSubtotal);
//   $('#add-total-diskon').val(subtotal);
// }
