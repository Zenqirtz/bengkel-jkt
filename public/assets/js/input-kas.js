/**
 * Input Kas - Page Data Management
 * Sama seperti Input Bank, bedanya tidak ada Bank & No. Rekening
 */
'use strict';
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const dt_basic_table = document.querySelector('.datatables-ik');
  ...
  if (dt_basic_table) {
    ...
    data-jenis="${full.jenis || ''}">`;
      ...
      initComplete: function () {
        const sa = document.getElementById('selectAllIK');
        if (sa) {
          sa.addEventListener('change', function () {
            document.querySelectorAll('.datatables-ik .dt-checkboxes').forEach(chk => {
    ...
    //   const chk = e.target.closest('.datatables-ik .dt-checkboxes');
    //   if (chk && chk.checked) $('.datatables-ik .dt-checkboxes').not(chk).prop('checked', false);
    // });
    ...
    document.querySelector('.add-new')?.addEventListener('click', function () {
      ...
      clearFormData();
      document.getElementById('ik_id').value = '';
    });
    ...
    document.querySelector('.edit-record')?.addEventListener('click', function () {
      ...
      // const chk = document.querySelector('.datatables-ik .dt-checkboxes:checked');
      // if (!chk) {
      //   alertPilihData();
      //   return;
      // }
      const checked = document.querySelectorAll('.datatables-ik .dt-checkboxes:checked');
      if (!checked.length) {
        alertPilihData();
        return;
      }
      ...
      clearFormData();
      fetch(`${baseUrl}input-kas-list/${chk.value}/edit`)
        .then(r => r.json())
        .then(res => {
          if (!res.status) {
            Swal.fire({
          ...
          document.getElementById('ik_id').value = d.id;
          ...
          // Nilai readonly saat ada gabung, unlock saat kosong
          // if (d.no_inv_gabung) {
    ...
    $(dt_basic_table).on('click', 'tbody tr', function (e) {
      ...
      showDetailInputKas(rowData.id);
    });

    function showDetailInputKas(id) {
      fetch(`${baseUrl}input-kas-list/${id}/edit`)
        .then(r => r.json())
        .then(res => {
          if (!res.status) {
            Swal.fire({
          ...
          document.getElementById('view-spk').value = d.no_spk || '';
          document.getElementById('view-nilai').value = d.nilai_fmt || '0';
...
`
    ...
    document.querySelector('.detail-record')?.addEventListener('click', function () {
      ...
      showDetailInputKas(checked[0].value);
    });
    ...
    document.querySelector('.delete-record')?.addEventListener('click', function () {
      ...
      Swal.fire({
        ...
        const hapusSatuPersatu = index => {
          ...
          fetch(`${baseUrl}input-kas-list/${ids[index]}`, {
            method: 'DELETE',
            headers: {
  ...
  document.querySelector('.cetak-record')?.addEventListener('click', function () {
   ...
   Silahkan pilih data dengan jenis yang sama!`,
    ...
    window.open(`${baseUrl}keuangan/cetak-input-kas?` + queryString, '_blank');
  });
  ...
  //   document.getElementById('add-nilai').value = Number(chk.dataset.total || 0).toLocaleString('en-US');
  //   hitungSisa();

  //   const instInv = bootstrap.Modal.getInstance(modalPilihInv);
  //   if (instInv) {
  //     modalPilihInv.addEventListener('hidden.bs.modal', function onBack() {
  //       modalPilihInv.removeEventListener('hidden.bs.modal', onBack);
  //       new bootstrap.Modal(addRoleModal).show();
  //     });
  //     instInv.hide();
  //   } else {
 ...
 Silahkan pilih data dengan nama yang sama!`,
  ...
  document.getElementById('btn-pilih-umj')?.addEventListener('click', function () {
    const editId = document.getElementById('ik_id')?.value || '';
    const instForm = bootstrap.Modal.getInstance(addRoleModal);
    if (instForm) {
  ...
  document.getElementById('btn-pilih-umj-terpilih')?.addEventListener('click', function () {
    ...
    populateUmjFields({
      ...
      nilai: Number(chk.dataset.nilai || 0),
      nilai_fmt: Number(chk.dataset.nilai || 0).toLocaleString('en-US'),
      jenis_penerimaan: chk.dataset.jenisPenerimaan || '',
      tanggal: chk.dataset.tanggal || '',
      dp_fmt: Number(chk.dataset.nilai || 0).toLocaleString('en-US')
    });
    hitungSisa();
    // lockNilai();
    ...
    if (instUmj) {
      modalPilihUmj.addEventListener('hidden.bs.modal', function onBack() {
        modalPilihUmj.removeEventListener('hidden.bs.modal', onBack);
        new bootstrap.Modal(addRoleModal).show();
      });
      instUmj.hide();
    } else {
  ...
  if (addNewDataForm) {
    ...
    // Reset form hanya jika modal ditutup beneran (bukan saat buka popup)
...
function _initInvDataTable(table) {
  return new DataTable(table, {
    ...
    columnDefs: [
      ...
      data-total="${full.total_nilai || 0}" data-nama="${full.nama || ''}" data-pph="${full.pph || 0}">`;
        }
      },
      {
        targets: 3,
...
function _loadInvToDataTable(tipe) {
  ...
  fetch(`${baseUrl}input-kas-list?${param}&search=`)
    .then(r => r.json())
    .then(data => {
...
function _loadUmjToDataTable(editId) {
  ...
  fetch(`${baseUrl}input-kas-list?${params.toString()}`)
    .then(r => r.json())
    .then(data => {
...
// function unlockGabung() {
//   const inp = document.getElementById('add-no-inv-gabung');
//   const btn = document.getElementById('btn-pilih-inv');
//   if (inp) {
...
function _loadHrlToDataTable() {
  ...
  fetch(`${baseUrl}input-kas-list?get_hrl=1`)
    .then(r => r.json())
    .then(data => {
