 ...
 */
 ...
 document.addEventListener('DOMContentLoaded', function (e) {
   ...
   const dt_basic_table = document.querySelector('.datatables-asuransi'),
       ...
       if (e.target.closest('.delete-record')) {
          if (isDelete) {
            ...
            Swal.fire({
              ...
              if (result.value) {
                // delete the data
                fetch(`${baseUrl}asuransi-list/${user_id}`, {
                  method: 'DELETE',
                  headers: {
     ...
     document.addEventListener('click', function (e) {
       if (e.target.closest('.edit-record')) {
          if(isEdit) {
            ...
            // get data
            fetch(`${baseUrl}asuransi-list/${user_id}/edit`)
              .then(response => response.json())
              .then(data => {
                document.getElementById('user_id').value = data.id;
                document.getElementById('add-nama').value = data.nama_pelanggan;
                document.getElementById('add-telepon').value = data.telepon;
    
                setSelectValue('#add-jenis',   data.kode_jenis_pelanggan,  data.kode_jenis_pelanggan_name);
                setSelectValue('#add-status',   data.is_active,  data.is_active_name);
              });
          } else {
            // Hide offcanvas
     ...
     const fv = FormValidation.formValidation(addNewDataForm, {
       fields: {
        nama_pelanggan: {
           validators: {
             notEmpty: {
               message: 'Silahkan Input Nama Customer'
             },
             stringLength: {
               max: 50,
         ...
         kode_jenis_pelanggan: {
           validators: {
             notEmpty: {
               message: 'Silahkan Input Jenis Customer'
             }
           }
         },
         is_active: {
