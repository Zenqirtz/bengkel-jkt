 ...
 */
 ...
 document.addEventListener('DOMContentLoaded', function (e) {
   ...
   const dt_basic_table = document.querySelector('.datatables-pelanggan'),
  ...
  if (dt_basic_table) {
    ...
    const dt_basic = new DataTable(dt_basic_table, {
      ...
      columns: [
        ...
        { data: 'id' },
        { data: 'id' },
        { data: 'nama_pelanggan', width: '300px' },
    ...
    document.addEventListener('click', function (e) {
      const chk = e.target.closest('.datatables-pelanggan .dt-checkboxes');
      if (chk) {
        if (chk.checked) {
    ...
    if (btnEditSelected) {
      btnEditSelected.addEventListener('click', function () {
        ...
        const selectedCheckbox = document.querySelector('.datatables-pelanggan .dt-checkboxes:checked');
        ...
        if (!selectedCheckbox) {
          ...
          // get data
          fetch(`${baseUrl}pelanggan-list/${user_id}/edit`)
          ...
          .then(data => {
            ...
            setSelectValue('#add-jenis-pelanggan',   data.kode_jenis_pelanggan,  '');
      ...
      btnDeletedSelected.addEventListener('click', function () {
        ...
        if (!selectedCheckbox) {
          ...
          Swal.fire({
            ...
            if (result.value) {
              // delete the data
              fetch(`${baseUrl}pelanggan-list/${user_id}`, {
                method: 'DELETE',
                headers: {
    ...
    document.addEventListener('click', function (e) {
      if (e.target.closest('.download-file')) {
        ...
        if(user_id.length) {
          ...
          const printUrl = `${baseUrl}customer-service/download-file-pelanggan?` + queryString;
    ...
    if (addNewBtn) {
      addNewBtn.addEventListener('click', function () {
        if(isAdd) {
          ...
          document.getElementById('add-nama-pelanggan')?.focus();
        } else {
          // Hide offcanvas
    ...
    const fv = FormValidation.formValidation(addNewDataForm, {
      fields: {
        nama_pelanggan: {
          validators: {
            notEmpty: {
              message: 'Silahkan Input Nama Pelanggan'
            },
            stringLength: {
              max: 50,
        ...
        kode_jenis_pelanggan: {
          validators: {
            notEmpty: {
              message: 'Silahkan Input Jenis Pelanggan'
            }
          }
        },
        telepon: {
      ...
      // const url = id ? `${baseUrl}pelanggan-list/${id}` : `${baseUrl}pelanggan-list`;

      // // jika update dengan method spoofing (Laravel)
      // if (id) fd.append('_method', 'PUT');

      fetch(`${baseUrl}pelanggan-list`, {
        method: 'POST',
        headers: {
          'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
 ...
 function clearFormData() {
    ...
    clearSelect('#add-jenis-pelanggan');
    clearSelect('#add-status-aktif');

    $('#add-nama-pelanggan').addClass('is-invalid');
    $('#add-jenis-pelanggan').addClass('is-invalid');
