 ...
 */
 ...
 document.addEventListener('DOMContentLoaded', function (e) {
   ...
   const dt_basic_table = document.querySelector('.datatables-tipe-kendaraan'),
  ...
  if (dt_basic_table) {
    ...
    const dt_basic = new DataTable(dt_basic_table, {
      ...
      scrollX: true,
      ajax: {
        url: baseUrl + 'tipe-kendaraan-list',
        data: function (d) {
          // Ambil data dari input form modal dan masukkan ke parameter request
          d.nama_tipe = $('#filter-tipe-kendaraan').val();
          d.kode_merek = $('#filter-merek-kendaraan').val();
          d.kode_jenis = $('#filter-jenis-kendaraan').val();
    ...
    document.addEventListener('click', function (e) {
      const chk = e.target.closest('.datatables-tipe-kendaraan .dt-checkboxes');
      if (chk) {
        if (chk.checked) {
    ...
    if (btnEditSelected) {
      btnEditSelected.addEventListener('click', function () {
        ...
        const selectedCheckbox = document.querySelector('.datatables-tipe-kendaraan .dt-checkboxes:checked');
        ...
        if (!selectedCheckbox) {
          ...
          // get data
          fetch(`${baseUrl}tipe-kendaraan-list/${user_id}/edit`)
          ...
          .then(data => {
            ...
            setSelectValue('#add-merek-kendaraan',   data.kode_merek,  '');
            setSelectValue('#add-jenis-kendaraan',   data.kode_jenis,  '');
      ...
      btnDeletedSelected.addEventListener('click', function () {
        ...
        if (!selectedCheckbox) {
          ...
          Swal.fire({
            ...
            if (result.value) {
              // delete the data
              fetch(`${baseUrl}tipe-kendaraan-list/${user_id}`, {
                method: 'DELETE',
                headers: {
    ...
    if (addNewBtn) {
      addNewBtn.addEventListener('click', function () {
        if(isAdd) {
          ...
          document.getElementById('add-tipe-kendaraan')?.focus();
        } else {
          // Hide offcanvas
    ...
    const fv = FormValidation.formValidation(addNewDataForm, {
      fields: {
        ...
        nama_tipe: {
          validators: {
            notEmpty: {
              message: 'Silahkan Input Tipe Kendaraan'
            },
            stringLength: {
              max: 50,
        ...
        is_active: {
          validators: {
            notEmpty: {
              message: 'Silahkan Input Status Aktif'
            }
          }
        }
      },
      ...
      fetch(`${baseUrl}tipe-kendaraan-list`, {
        method: 'POST',
        headers: {
          'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
 ...
 function clearFormData() {
    ...
    $('#add-merek-kendaraan').addClass('is-invalid');
    $('#add-jenis-kendaraan').addClass('is-invalid');
    $('#add-tipe-kendaraan').addClass('is-invalid');
