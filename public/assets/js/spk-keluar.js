 ...
 */
       ...
       if (e.target.closest('.delete-record')) {
          if (isDelete) {
            ...
            Swal.fire({
              ...
              if (result.value) {
                // delete the data
                fetch(`${baseUrl}spk-keluar-list/${user_id}`, {
                  method: 'DELETE',
                  headers: {
     ...
     document.addEventListener('click', function (e) {
       if (e.target.closest('.edit-record')) {
          if(isEdit) {
            ...
            // get data
            fetch(`${baseUrl}spk-keluar-list/${user_id}/edit`)
          ...
          if (!selectedCheckbox) {
            ...
            .then(result => {
              ...
              if(status) {
                ...
                document.getElementById('add-nomor-keluar').value = data.kode_keluar;
                document.getElementById('add-tgl-keluar').value = data.tgl_keluar;
                document.getElementById('add-penerima').value = data.nama_penerima;
                document.getElementById('add-tgl-terima').value = data.tgl_tanda_terima;
                document.getElementById('add-pengantar').value = data.nama_pengantar;
      ...
      const btnEditSelected2 = document.querySelector('.cetak-kendaraan-keluar');
      if (btnEditSelected2) {
        btnEditSelected2.addEventListener('click', function () {
          ...
          if (!selectedCheckbox) {
            ...
            const printUrl = `${baseUrl}customer-service/cetak-spk-keluar?` + queryString;
      ...
      const FormValidation2 = FormValidation.formValidation(wizardPropertyListingFormStep2, {
        fields: {
          kode_keluar: {
            validators: {
              notEmpty: {
                message: 'Silahkan Input Nomor Keluar'
              }
            }
          },
          tgl_keluar: {
            validators: {
              notEmpty: {
                message: 'Silahkan Input Tanggal Keluar'
              }
            }
          },
          nama_penerima: {
            validators: {
              notEmpty: {
                message: 'Silahkan Input Nama Penerima'
              }
            }
          },
          tgl_tanda_terima: {
 ...
 function clearFormData() {
  ...
  $('#add-nomor-keluar').addClass('is-invalid');
  $('#add-tgl-keluar').addClass('is-invalid');
  $('#add-penerima').addClass('is-invalid');
  $('#add-tgl-terima').addClass('is-invalid');
