 ...
 */
 ...
 document.addEventListener('DOMContentLoaded', function (e) {
   ...
   const dt_basic_table = document.querySelector('.datatables-menu'),
   ...
   if (dt_basic_table) {
    ...
    const dt_basic = new DataTable(dt_basic_table, {
      ...
      ajax: {
        ...
        data: function (d) {
          ...
          d.parent_id = $('#filter-sub-menu').val();
          d.custom_title = $('#filter-menu').val();
          d.active = $('#filter-status-aktif').val();
        },
    ...
    document.addEventListener('click', function (e) {
      const chk = e.target.closest('.datatables-menu .dt-checkboxes');
      if (chk) {
        if (chk.checked) {
    ...
    if (btnEditSelected) {
      btnEditSelected.addEventListener('click', function () {
        ...
        const selectedCheckbox = document.querySelector('.datatables-menu .dt-checkboxes:checked');
        ...
        if (!selectedCheckbox) {
          ...
          // get data
          fetch(`${baseUrl}menu-list/${user_id}/edit`)
          ...
          .then(data => {
            ...
            document.getElementById('add-menu').value = data.custom_title;
            document.getElementById('add-url').value = data.url_menu;
            document.getElementById('add-slug').value = data.slug;
            document.getElementById('add-path-icon').value = data.path_icon;
            document.getElementById('add-posisi').value = data.tid;

            setSelectValue('#add-sub-menu', data.parent_id, '');
            setSelectValue('#add-status-aktif', data.active, '');
          });
        }
      });
    }
      ...
      btnDeletedSelected.addEventListener('click', function () {
        ...
        if (!selectedCheckbox) {
          ...
          Swal.fire({
            ...
            if (result.value) {
              // delete the data
              fetch(`${baseUrl}menu-list/${user_id}`, {
                method: 'DELETE',
                headers: {
    ...
    document.addEventListener('click', function (e) {
      if (e.target.closest('.download-file')) {
        ...
        if(user_id.length) {
          ...
          const printUrl = `${baseUrl}setting/download-file-menu?` + queryString;
    ...
    if (addNewBtn) {
      ...
      if(isAdd) {
        ...
        document.getElementById('add-sub-menu')?.focus();
      } else {
        // Hide offcanvas
     ...
     const fv = FormValidation.formValidation(addNewDataForm, {
       fields: {
        parent_id: {
          validators: {
            notEmpty: {
              message: 'Silahkan Input Sub Menu'
            }
          }
        },
        custom_title: {
          validators: {
            notEmpty: {
              message: 'Silahkan Input Nama Menu'
            }
          }
        },
        url_menu: {
 ...
 function clearFormData() {
    ...
    clearSelect('#add-sub-menu');
    clearSelect('#add-status-aktif');

    $('#add-sub-menu').addClass('is-invalid');
    $('#add-menu').addClass('is-invalid');
