...
document.addEventListener('DOMContentLoaded', function (e) {
  ...
  if (dt_basic_table) {
    ...
    const dt_basic = new DataTable(dt_basic_table, {
      ...
      serverSide: true,
      ajax: {
        url: baseUrl + 'tanda-terima-invoice-list',
        data: function (d) {
      ...
      columnDefs: [
        ...
        //  },
        {
          ...
          render: function (data, type, full, meta) {
            ...
            value="${data}"
            data-kode-pelanggan="${full.kode_pelanggan}"
            data-nama-pelanggan="${full.nama_pelanggan}">`;
          },
          checkboxes: {
      ...
      if (e.target.closest('.delete-record')) {
        if (isDelete) {
          ...
          Swal.fire({
            ...
            if (result.value) {
              // delete the data
              fetch(`${baseUrl}tanda-terima-invoice-list/${user_id}`, {
                method: 'DELETE',
                headers: {
    ...
    document.addEventListener('click', function (e) {
      if (e.target.closest('.edit-record')) {
        if (isEdit) {
          ...
          // get data
          fetch(`${baseUrl}tanda-terima-invoice-list/${user_id}/edit`)
    ...
    //             const printUrl = `${baseUrl}administrasi/cetak-tanda-terima-invoice?` + queryString;
    ...
    if (btnEditSelected2) {
      btnEditSelected2.addEventListener('click', function () {
        ...
        const kodePelangganList = [...new Set(Array.from(selectedCheckboxes).map(chk => chk.dataset.kodePelanggan))];
        const namaPelangganList = [...new Set(Array.from(selectedCheckboxes).map(chk => chk.dataset.namaPelanggan))];

        if (kodePelangganList.length > 1) {
          Swal.fire({
            icon: 'warning',
            title: 'Peringatan',
            html: `Data yang dipilih memiliki nama pelanggan berbeda:<br><br>
           <b>${namaPelangganList.join('<br>')}</b><br><br>
           Silahkan pilih data dengan pelanggan yang sama!`,
            customClass: { confirmButton: 'btn btn-primary' }
          });
          return;
        }
  ...
  if (typeof wizardPropertyListing !== undefined && wizardPropertyListing !== null) {
    ...
    const FormValidation2 = FormValidation.formValidation(wizardPropertyListingFormStep2, {
      ...
      fetch(`${baseUrl}tanda-terima-invoice-list`, {
        method: 'POST',
        headers: {
          'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
