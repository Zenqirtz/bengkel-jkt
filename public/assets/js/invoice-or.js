       ...
       if (e.target.closest('.delete-record')) {
          if (isDelete) {
            ...
            Swal.fire({
              ...
              if (result.value) {
                // delete the data
                fetch(`${baseUrl}invoice-or-list/${user_id}`, {
                  method: 'DELETE',
                  headers: {
     ...
     document.addEventListener('click', function (e) {
       if (e.target.closest('.edit-record')) {
          if(isEdit) {
            ...
            // get data
            fetch(`${baseUrl}invoice-or-list/${user_id}/edit`)
          ...
          if (!selectedCheckbox) {
            ...
            .then(result => {
              ...
              if(status) {
                ...
                setSelectValue('#add-or-fee', data.is_free,  '');
          ...
          .then(result => {
            ...
            if(status) {
              ...
              if(data.no_invoice) {
                ...
                const printUrl = `${baseUrl}customer-service/cetak-invoice-or?` + queryString;
                ...
                Swal.fire({
                  icon: 'warning',
                  title: `Peringatan!`,
                  text: `Nomor Invoice belum dibuat!`,
                  customClass: {
                    confirmButton: 'btn btn-success'
                  }
                });
...
// Handle Cetak Filter Per Pelanggan
    const formCetakFilter = document.getElementById('formCetakFilter');
    if (formCetakFilter) {
      formCetakFilter.addEventListener('submit', function (e) {
        e.preventDefault();

        // Kumpulkan semua value dari form filter cetak
        const params = {
          nama_pelanggan   : document.getElementById('cetak-nama-pelanggan').value,
          kode_spk         : document.getElementById('cetak-nomor-spk').value,
          no_polisi        : document.getElementById('cetak-no-polisi').value,
          no_polis         : document.getElementById('cetak-no-polis').value,
          kode_claim       : document.getElementById('cetak-no-klaim').value,
          tgl_invoice_awal : document.getElementById('cetak-tgl-invoice-awal').value,
          tgl_invoice_akhir: document.getElementById('cetak-tgl-invoice-akhir').value,
          status_spk       : $('#cetak-status-spk').val() || 'all',
        };

        // Hapus parameter yang kosong supaya URL bersih
        Object.keys(params).forEach(key => {
          if (!params[key]) delete params[key];
        });

        const queryString = $.param(params);
        const printUrl = `${baseUrl}customer-service/cetak-invoice-or-filter?` + queryString;

        // Tutup modal dulu
        const modalEl = document.getElementById('cetakFilterModal');
