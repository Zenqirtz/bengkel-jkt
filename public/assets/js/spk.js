 ...
 */
 ...
 document.addEventListener('DOMContentLoaded', function (e) {
  ...
  isDelete = dt_basic_table.dataset.delete;
       ...
       if (e.target.closest('.delete-record')) {
          if (isDelete) {
            ...
            Swal.fire({
              ...
              if (result.value) {
                // delete the data
                fetch(`${baseUrl}spk-list/${user_id}`, {
                  method: 'DELETE',
                  headers: {
     ...
     document.addEventListener('click', function (e) {
       if (e.target.closest('.edit-record')) {
          if(isEdit) {
            ...
            // get data
            fetch(`${baseUrl}spk-list/${user_id}/edit`)
   ...
   if (select2.length) {
    ...
    const selectPemilik = $('#add-nopolisi'); // ID dari select box
    if (selectPemilik.length) {
      selectPemilik.on('change', function () {
        ...
        const inputNama   = $('#add-nama-pemilik');
        const inputHp     = $('#add-handphone-pemilik');
        const inputTlpn   = $('#add-telepon-pemilik');
        const inputAlamat = $('#add-alamat-pemilik');
        const inputMerek  = $('#add-merek-kendaraan');
        const inputJenis  = $('#add-jenis-kendaraan');
        const inputTipe   = $('#add-tipe-kendaraan');
        const inputWarna  = $('#add-warna');
        const inputTertanggung  = $('#add-tertanggung');
        ...
        if (id) {
          ...
          fetch(`${baseUrl}customer-service/get-kendaraan/${id}`) // Sesuaikan URL dengan route di langkah 1
            .then(response => {
              if (!response.ok) {
            ...
            .then(data => {
              ...
              inputMerek.val(data.nama_merek || '-'); 
              inputJenis.val(data.nama_jenis || '-'); 
              inputTipe.val(data.nama_tipe || '-'); 
              inputWarna.val(data.warna || '-'); 
              inputTertanggung.val(data.nama_pemilik || '-'); 
      ...
      const FormValidation2 = FormValidation.formValidation(wizardPropertyListingFormStep2, {
        fields: {
          kode_status_spk: {
            validators: {
              notEmpty: {
                message: 'Silahkan Input Status SPK'
              }
            }
          },
          kode_jenis_pelanggan: {
            validators: {
              notEmpty: {
                message: 'Silahkan Input Jenis Asuransi'
              }
            }
          }
        },
    ...
    const selectJenis = $('#add-jenis-asuransi');
    if (selectJenis.length) {
      selectJenis.on('change', function () {
        const id = $(this).val();
        if(id == "00001") { // Asuransi
          $("#add-jenis-polis").prop('disabled', false);
          $("#add-no-polis").prop('disabled', false);
          $("#add-no-klaim").prop('disabled', false);
          $("#add-tgl-polis").prop('disabled', false);
          $("#add-jenis-perbaikan").prop('disabled', false);
        } else if(id == "00002") { // Non Asuransi {
          $("#add-jenis-polis").prop('disabled', true);
          $("#add-no-polis").prop('disabled', true);
          $("#add-no-klaim").prop('disabled', true);
          $("#add-tgl-polis").prop('disabled', true);
          $("#add-jenis-perbaikan").prop('disabled', true);
        }
        
        fetchNamaAsuransi('#add-jenis-asuransi', '#add-pelanggan');
      });
    }
 });

  function fetchNamaAsuransi(selectorJenis, selectorAsuransi) {
    const jenisId = $(selectorJenis).val();
    const selectAsuransi = $(selectorAsuransi);
    ...
    if (jenisId) {
      ...
      selectAsuransi.prop('disabled', true);
      
      // Gunakan helper clearSelect yg sudah ada di kode Anda untuk reset opsi lama
      clearSelect('#add-pelanggan', { keepOptions: false }); 

      // Fetch data
      fetch(`${baseUrl}customer-service/get-nama-asuransi?jenis_id=${jenisId}`)
        .then(response => {
        ...
        .then(data => {
          ...
          const defaultOption = new Option('Pilih Nama Asuransi', '', true, true);
          selectAsuransi.append(defaultOption).trigger('change');

          // Loop data dari server dan masukkan ke select
          data.forEach(item => {
            // Cek apakah kode_bahan saat ini sama dengan yang dicari
            var isSelected = (item.kode_pelanggan == kode_pelanggan);

            // Pastikan key 'item.id' dan 'item.nama_tipe' sesuai return Controller
            const newOption = new Option(item.nama_pelanggan, item.kode_pelanggan, false, isSelected);
            selectAsuransi.append(newOption);
          });

          // Refresh Select2 agar opsi baru muncul
          selectAsuransi.trigger('change');
          selectAsuransi.prop('disabled', false); // Aktifkan kembali
        })
        .catch(error => {
          console.error('Error:', error);
          selectAsuransi.prop('disabled', false);
        });
 ...
 function clearSelect(selector, { keepOptions = true } = {}) {
  ...
  function clearFormData() {
    ...
    clearSelect('#add-status-spk');
    clearSelect('#add-jenis-asuransi');
    clearSelect('#add-pelanggan');
    clearSelect('#add-perantara');
    clearSelect('#add-marketing');
    clearSelect('#add-jenis-polis');

    $('#add-nopolisi').addClass('is-invalid');
    $('#add-status-spk').addClass('is-invalid');
    $('#add-jenis-asuransi').addClass('is-invalid');
