 ...
 */
 ...
 document.addEventListener('DOMContentLoaded', function (e) {
  let isAdd, isEdit, isDelete;
   ...
   const dt_inv_belum_terbit_table = document.querySelector('.datatables-inv-belum-terbit');
   if (dt_inv_belum_terbit_table) {
     isAdd = dt_inv_belum_terbit_table.dataset.add;
     const dt_inv_belum_terbit = new DataTable(dt_inv_belum_terbit_table, {
       ...
       scrollX: true,
       ajax: {
         url: baseUrl + 'laporan-invoice-list',
         data: function (d) {
    ...
    const dt_inv_terbit_table = document.querySelector('.datatables-inv-terbit');
    if (dt_inv_terbit_table) {
      isAdd = dt_inv_terbit_table.dataset.add;
      const dt_inv_terbit = new DataTable(dt_inv_terbit_table, {
        searching: false,  // Opsi ini akan menghilangkan input cari
        ordering: false,    // Opsi ini akan menghilangkan ordering
        paging: false,    // Opsi ini akan menghilangkan paging
        processing: true,
    ...
    const dt_inv_belum_tagih_table = document.querySelector('.datatables-inv-belum-tagih');
    if (dt_inv_belum_tagih_table) {
      isAdd = dt_inv_belum_tagih_table.dataset.add;
      const dt_inv_belum_tagih = new DataTable(dt_inv_belum_tagih_table, {
        searching: false,  // Opsi ini akan menghilangkan input cari
        ordering: false,    // Opsi ini akan menghilangkan ordering
        paging: false,    // Opsi ini akan menghilangkan paging
        processing: true,
    ...
    const dt_inv_belum_lunas_table = document.querySelector('.datatables-inv-belum-lunas');
    if (dt_inv_belum_lunas_table) {
      isAdd = dt_inv_belum_lunas_table.dataset.add;
      const dt_inv_belum_lunas = new DataTable(dt_inv_belum_lunas_table, {
        searching: false,  // Opsi ini akan menghilangkan input cari
        ordering: false,    // Opsi ini akan menghilangkan ordering
        paging: false,    // Opsi ini akan menghilangkan paging
        processing: true,
   ...
   if (exportBtn) {
      exportBtn.addEventListener('click', function () {
        if(isAdd) {
          ...
          window.location.href = `${baseUrl}laporan-invoice/export?` + queryString;
        } else {
          // Hide offcanvas
   ...
   if (printBtn) {
    printBtn.addEventListener('click', function () {
        if(isAdd) {
          ...
          // Redirect window untuk download file
          // Pastikan route URL sesuai dengan konfigurasi route Anda
          // window.location.href = `${baseUrl}laporan-invoice/print?` + queryString;
          const printUrl = `${baseUrl}laporan-invoice/print?` + queryString;
