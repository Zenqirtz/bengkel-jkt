...
document.addEventListener('DOMContentLoaded', function (e) {
  ...
  const dt_rekap_table = document.querySelector('.datatables-aging-penagihan-rekap');
  if (dt_rekap_table) {
    ...
    const dt_rekap = new DataTable(dt_rekap_table, {
      ...
      scrollX: true,
      ajax: {
        url: baseUrl + 'laporan-aging-penagihan-list',
        data: function (d) {
  ...
  const dt_rinci_table = document.querySelector('.datatables-aging-penagihan-rinci');
  if (dt_rinci_table) {
    ...
    drawCallback: function (settings) {
      ...
      api.column(16, { page: 'current' }).data().each(function (asuransi, i) {
        // Ambil nilai Index ke-15 (minggu / Keterangan)
        var keterangan = api.column(17, { page: 'current' }).data()[i];

        // Grouping Level 1: Nama Asuransi
        if (lastAsuransi !== asuransi) {
          $(rows).eq(i).before(
            '<tr class="group"><td colspan="16" style="background-color: #d7e4ff; font-weight: bold; color: #000;">' + asuransi + '</td></tr>'
          );
        ...
        if (lastKeterangan !== keterangan) {
          $(rows).eq(i).before(
            '<tr class="group"><td colspan="16" style="background-color: #f0f5ff; padding-left: 30px; font-weight: 600; color: #333;">' + keterangan + '</td></tr>'
          );
  ...
  if (exportBtn) {
    exportBtn.addEventListener('click', function () {
      if (isAdd) {
        ...
        window.location.href = `${baseUrl}laporan-aging-penagihan/export?${queryString}`;
      } else {
        Swal.fire({
          icon: 'error',
          title: 'Error!',
  ...
  if (printBtn) {
    printBtn.addEventListener('click', function () {
      if (isAdd) {
        ...
        let queryString = $.param(params);
        const printUrl = `${baseUrl}laporan-aging-penagihan/print?${queryString}`;
