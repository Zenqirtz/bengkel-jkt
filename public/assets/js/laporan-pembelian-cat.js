...
document.addEventListener('DOMContentLoaded', function (e) {
  ...
  const dt_pembelian_cat_table = document.querySelector('.datatables-pembelian-cat');
  if (dt_pembelian_cat_table) {
    isAdd = dt_pembelian_cat_table.dataset.add;

    dt_pembelian_cat = new DataTable(dt_pembelian_cat_table, {
      searching: false,
      ordering: false,
      paging: false,
      processing: true,
      ...
      scrollX: true,
      ajax: {
        url: baseUrl + 'laporan-pembelian-cat-list',
        data: function (d) {
  ...
  if (exportBtn) {
    exportBtn.addEventListener('click', function () {
      if (isAdd) {
        ...
        window.location.href = `${baseUrl}laporan-pembelian-cat/export?` + queryString;
      } else {
        Swal.fire({
          icon: 'error',
          title: 'Error!',
  ...
  if (printBtn) {
    printBtn.addEventListener('click', function () {
      if (isAdd) {
        ...
        let queryString = $.param(params);
        const printUrl = `${baseUrl}laporan-pembelian-cat/print?` + queryString;
