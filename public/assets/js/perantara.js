 ...
 */
 ...
 document.addEventListener('DOMContentLoaded', function (e) {
   ...
   const dt_basic_table = document.querySelector('.datatables-perantara'),
  ...
  if (dt_basic_table) {
    ...
    const dt_basic = new DataTable(dt_basic_table, {
      ...
      scrollX: true,
      ajax: {
        url: baseUrl + 'perantara-list',
        data: function (d) {
          // Ambil data dari input form modal dan masukkan ke parameter request
          d.nama_perantara = $('#filter-nama-perantara').val();
          d.kode_jenis_perantara = $('#filter-jenis-perantara').val();
          d.is_active = $('#filter-status-aktif').val();
          d.status = $('#filter-status-perantara').val();
      ...
      columns: [
        ...
        { data: 'id' },
        { data: 'nama_perantara' },
        { data: 'jenis_perantara' },
        { data: 'status_perantara' },
        { data: 'telepon' },
        { data: 'status_aktif' },
      ],
      columnDefs: [
    ...
    document.addEventListener('click', function (e) {
      const chk = e.target.closest('.datatables-perantara .dt-checkboxes');
      if (chk) {
        if (chk.checked) {
    ...
    if (btnEditSelected) {
      btnEditSelected.addEventListener('click', function () {
        ...
        const selectedCheckbox = document.querySelector('.datatables-perantara .dt-checkboxes:checked');
        ...
        if (!selectedCheckbox) {
          ...
          // get data
          fetch(`${baseUrl}perantara-list/${user_id}/edit`)
          ...
          .then(data => {
            ...
            document.getElementById('add-nama-perantara').value = data.nama_perantara;
            document.getElementById('add-telepon').value = data.telepon;

            setSelectValue('#add-jenis-perantara',   data.kode_jenis_perantara,  '');
      ...
      btnDeletedSelected.addEventListener('click', function () {
        ...
        if (!selectedCheckbox) {
          ...
          Swal.fire({
            ...
            if (result.value) {
              // delete the data
              fetch(`${baseUrl}perantara-list/${user_id}`, {
                method: 'DELETE',
                headers: {
    ...
    if (addNewBtn) {
      addNewBtn.addEventListener('click', function () {
        if(isAdd) {
          ...
          document.getElementById('add-nama-perantara')?.focus();
        } else {
          // Hide offcanvas
    ...
    const fv = FormValidation.formValidation(addNewDataForm, {
      fields: {
        nama_perantara: {
          validators: {
            notEmpty: {
              message: 'Silahkan Input Nama Perantara'
            },
            stringLength: {
              max: 50,
        ...
        kode_jenis_perantara: {
          validators: {
            notEmpty: {
              message: 'Silahkan Input Jenis Perantara'
            }
          }
        },
        telepon: {
 ...
 function clearFormData() {
    ...
    clearSelect('#add-jenis-perantara');
    clearSelect('#add-status-aktif');

    $('#add-nama-perantara').addClass('is-invalid');
    $('#add-jenis-perantara').addClass('is-invalid');
