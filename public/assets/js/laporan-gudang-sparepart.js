...
document.addEventListener('DOMContentLoaded', function (e) {
  ...
  const dt_gudang_sparepart_table = document.querySelector('.datatables-gudang-sparepart');
  if (dt_gudang_sparepart_table) {
    isAdd = dt_gudang_sparepart_table.dataset.add;

    // dt_gudang_sparepart = new DataTable(dt_gudang_sparepart_table, {
    //   searching: false,
    //   ordering: false,
    //   paging: false,
    //   processing: true,
    ...
    //   scrollX: true,
    //   ajax: {
    //     url: baseUrl + 'laporan-gudang-sparepart-list',
    //     data: function (d) {
    ...
    //         'font-weight': 'bold'
    //       });
    //       $('td', row).eq(8).css('white-space', 'nowrap');
    //     }
    //   }
    // });
    dt_gudang_sparepart = new DataTable(dt_gudang_sparepart_table, {
      ...
      displayLength: 10, // TAMBAHAN: default page length
      language: {
        emptyTable: 'No data available in table',
        zeroRecords: 'No data available in table',
        paginate: {
          // TAMBAHAN: ikon paginate seragam dgn SPK
  ...
  if (exportBtn) {
    exportBtn.addEventListener('click', function () {
      if (isAdd) {
        ...
        window.location.href = `${baseUrl}laporan-gudang-sparepart/export?` + queryString;
      } else {
        Swal.fire({
          icon: 'error',
          title: 'Error!',
  ...
  if (printBtn) {
    printBtn.addEventListener('click', function () {
      if (isAdd) {
        ...
        let queryString = $.param(params);
        const printUrl = `${baseUrl}laporan-gudang-sparepart/print?` + queryString;
