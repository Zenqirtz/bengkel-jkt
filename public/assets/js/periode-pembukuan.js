 ...
 */
 ...
 document.addEventListener('DOMContentLoaded', function (e) {
   ...
   const dt_basic_table = document.querySelector('.datatables-periode-pembukuan'),
  ...
  if (dt_basic_table) {
    ...
    const dt_basic = new DataTable(dt_basic_table, {
      ...
      scrollX: true,
      ajax: {
        url: baseUrl + 'periode-pembukuan-list',
        data: function (d) {
    ...
    document.addEventListener('click', function (e) {
      const chk = e.target.closest('.datatables-periode-pembukuan .dt-checkboxes');
      if (chk) {
        if (chk.checked) {
    ...
    if (btnEditSelected) {
      btnEditSelected.addEventListener('click', function () {
        ...
        const selectedCheckbox = document.querySelector('.datatables-periode-pembukuan .dt-checkboxes:checked');
        ...
        if (!selectedCheckbox) {
          ...
          // get data
          fetch(`${baseUrl}periode-pembukuan-list/${user_id}/edit`)
          ...
          .then(data => {
            ...
            document.getElementById('add-tanggal').value = data.tgl_periode;
      ...
      btnDeletedSelected.addEventListener('click', function () {
        ...
        if (!selectedCheckbox) {
          ...
          Swal.fire({
            ...
            if (result.value) {
              // delete the data
              fetch(`${baseUrl}periode-pembukuan-list/${user_id}`, {
                method: 'DELETE',
                headers: {
    ...
    const fv = FormValidation.formValidation(addNewDataForm, {
      fields: {
        tgl_periode: {
          validators: {
            notEmpty: {
              message: 'Silahkan Input Tanggal Pembukuan'
            },
            between: {
              min: 1,
              max: 31,
              message: 'Input Angka antara 1 s/d 31'
            }
          }
        },
        keterangan: {
          validators: {
            notEmpty: {
              message: 'Silahkan Input Keteranganr'
            }
          }
        }
      },
      ...
      fetch(`${baseUrl}periode-pembukuan-list`, {
        method: 'POST',
        headers: {
          'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
