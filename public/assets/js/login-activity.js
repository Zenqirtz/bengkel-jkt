...
document.addEventListener('DOMContentLoaded', function () {
  let dt_login_activity;

  $.ajaxSetup({
    headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') }
  });

  const dt_table = document.querySelector('.datatables-login-activity');

  if (dt_table) {
    dt_login_activity = new DataTable(dt_table, {
      processing: true,
      serverSide: true,
      scrollX: true,
      ajax: {
        url: baseUrl + 'login-activity-list',
        type: 'GET',
        data: function (d) {
          d.tgl_awal          = $('#filter-tgl-awal').val();
          d.tgl_akhir         = $('#filter-tgl-akhir').val();
          d.userid            = $('#filter-userid').val();
          d.session_loginflag = $('#filter-session-loginflag').val();
        },
      ...
      columns: [
        ...
        { data: 'userid',             className: 'text-center', orderable: true },
        { data: 'session_startlogin', className: 'text-center', orderable: true },
        { data: 'session_endlogin',   className: 'text-center', orderable: true },
        { data: 'session_loginflag',  className: 'text-center', orderable: true },
        { data: 'session_closeby',    className: 'text-center', orderable: true },
      ],
      order: [[2, 'desc']], // default sort: Waktu Login descending
      layout: {
        topStart: {
  ...
  if (filterForm) {
    ...
    FormValidation.formValidation(filterForm, {
      ...
      if (dt_login_activity) {
        dt_login_activity.ajax.reload();
      }
    });
  }
});
