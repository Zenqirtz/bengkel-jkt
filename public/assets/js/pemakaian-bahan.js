...
document.addEventListener('DOMContentLoaded', function (e) {
  ...
  const dt_basic_table = document.querySelector('.datatables-pemakaian-bahan'),
  ...
  if (dt_basic_table) {
    ...
    const dt_basic = new DataTable(dt_basic_table, {
      ...
      scrollX: true,
      ajax: {
        url: baseUrl + 'pemakaian-bahan-list',
        data: function (d) {
          // Ambil data dari input form modal dan masukkan ke parameter request
          d.kode_posisi = $('#filter-pekerjaan').val();
          d.nama_bahan = $('#filter-bahan').val();
          d.point_panel = $('#filter-point-panel').val();
        },
    ...
    // Klik row tabel -> langsung tampilkan detail
    $(dt_basic_table).on('click', 'tbody tr', function (e) {
      if ($(e.target).is('input') || $(e.target).closest('td').index() === 0) {
        return;
      }
      const rowData = dt_basic.row(this).data();
      if (!rowData || !rowData.id) return;
      showDetailPemakaian(rowData.id);
    });

    // Batasi hanya 1 checkbox yang boleh dipilih
    document.addEventListener('click', function (e) {
      const chk = e.target.closest('.datatables-pemakaian-bahan .dt-checkboxes');
      if (chk) {
        if (chk.checked) {
    ...
    if (btnEditSelected) {
      btnEditSelected.addEventListener('click', function () {
        ...
        const selectedCheckbox = document.querySelector('.datatables-pemakaian-bahan .dt-checkboxes:checked');
        ...
        if (!selectedCheckbox) {
          ...
          // get data
          fetch(`${baseUrl}pemakaian-bahan-list/${user_id}/edit`)
            ...
            .then(data => {
              ...
              document.getElementById('kode_bahan2').value = data.kode_bahan;
              document.getElementById('add-qty').value = data.qty;

              setSelectValue('#add-pekerjaan', data.kode_posisi, '');
              setSelectValue('#add-bahan', data.kode_bahan, '');
              setSelectValue('#add-point-panel', data.point_panel, '');
            });
        }
      });
    }
      ...
      btnDeletedSelected.addEventListener('click', function () {
        ...
        if (!selectedCheckbox) {
          ...
          Swal.fire({
            ...
            if (result.value) {
              // delete the data
              fetch(`${baseUrl}pemakaian-bahan-list/${user_id}`, {
                method: 'DELETE',
                headers: {
    ...
    // // ── TAMBAH DI SINI ── Detail Record
    // const btnDetailSelected = document.querySelector('.detail-record');
    // if (btnDetailSelected) {
    //   btnDetailSelected.addEventListener('click', function () {
    //     const selectedCheckbox = document.querySelector('.datatables-pemakaian-bahan .dt-checkboxes:checked');
    ...
    //       .then(response => response.json())
    //       .then(data => {
    //         // Isi info header di modal
    //         document.getElementById('view-pekerjaan').value = data.posisi_pekerjaan ?? data.kode_posisi;
    //         document.getElementById('view-point-panel').value = data.point_panel;

    //         // Fetch semua bahan dengan kode_posisi + point_panel yang sama
    //         const params = new URLSearchParams({
    //           draw: 1,
    //           start: 0,
    //           length: 100,
    //           kode_posisi: data.kode_posisi,
    //           point_panel: data.point_panel
    //         });

    //         fetch(`${baseUrl}pemakaian-bahan-list/${user_id}/edit`)
    //           .then(response => response.json())
    //           .then(data => {
    //             document.getElementById('view-pekerjaan').value = data.posisi_pekerjaan;
    //             document.getElementById('view-point-panel').value = data.point_panel;

    //             // Langsung pakai data dari edit, tidak perlu fetch lagi
    //             const tbody = document.getElementById('detail-tbody');
    //             tbody.innerHTML = `
    //             <tr>
    //               <td style="width:5%">1</td>
    //               <td>${data.nama_bahan}</td>
    //               <td style="width:15%; text-align:right">${data.qty}</td>
    //             </tr>`;
    //             const modalInstance = new bootstrap.Modal(document.getElementById('viewRoleModal'));
    //             modalInstance.show();
    //           })
    //           .catch(() => {
    //             Swal.fire({
    ...
    // ── Fungsi Detail, dipanggil dari klik row ──
    function showDetailPemakaian(user_id) {
      fetch(`${baseUrl}pemakaian-bahan-list/${user_id}/edit`)
        .then(response => response.json())
        .then(data => {
          document.getElementById('view-pekerjaan').value = data.posisi_pekerjaan ?? data.kode_posisi;
    ...
    const fv = FormValidation.formValidation(addNewDataForm, {
      fields: {
        kode_posisi: {
          validators: {
            notEmpty: {
              message: 'Silahkan Input Pekerjaan'
            }
          }
        },
        kode_bahan: {
          validators: {
            notEmpty: {
              message: 'Silahkan Input Nama Bahan'
            }
          }
        },
        point_panel: {
          validators: {
            notEmpty: {
              message: 'Silahkan Input Point Panel'
            }
          }
        },
        qty: {
      ...
      // const url = id ? `${baseUrl}pemakaian-bahan-list/${id}` : `${baseUrl}pemakaian-bahan-list`;

      // // jika update dengan method spoofing (Laravel)
      // if (id) fd.append('_method', 'PUT');

      fetch(`${baseUrl}pemakaian-bahan-list`, {
        method: 'POST',
        headers: {
          'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
        ...
        .then(async response => {
          ...
          try {
            ...
            json = { status: false, message: text || 'Gagal' };
          }
  ...
  const selectBahan = $('#add-pekerjaan');
  if (selectBahan.length) {
    selectBahan.on('change', function () {
      // const kdPosisi = $(this).val();
      fetchNamaBahan('#add-pekerjaan', '#add-bahan');
    });
  }
});
...
function clearFormData() {
  ...
  clearSelect('#add-pekerjaan');
  clearSelect('#add-bahan');
  clearSelect('#add-point-panel');

  $('#add-pekerjaan').addClass('is-invalid');
  $('#add-bahan').addClass('is-invalid');
  $('#add-point-panel').addClass('is-invalid');
  $('#add-qty').addClass('is-invalid');
