 ...
 */
 ...
 document.addEventListener('DOMContentLoaded', function (e) {
  ...
  if (dt_basic_table) {
    ...
    //   const checkboxList = document.querySelectorAll('.datatables-salvage .dt-checkboxes');
    //   if (chk) {
    ...
    if (btnEditSelected) {
      btnEditSelected.addEventListener('click', function () {
        ...
        if (!selectedCheckbox) {
          ...
          // get data
          fetch(`${baseUrl}salvage-list/${user_id}/edit`)
          .then(response => {
            if (!response.ok) {
          ...
          .then(result => {
            ...
            if(status) {
              ...
              document.getElementById('user_id').value = data.salvage_id;
              document.getElementById('kode_spk').value = data.kode_spk;
              document.getElementById('no_polisi').value = data.no_polisi;
              document.getElementById('no_polis').value = data.no_polis;
              document.getElementById('kode_merek').value = data.kode_merek;
              document.getElementById('kode_tipe').value = data.kode_tipe;
              document.getElementById('kode_pelanggan').value = data.kode_pelanggan;

              document.getElementById('add-nomor-salvage').value = data.no_salvage;
              document.getElementById('add-tanggal-salvage').value = data.tgl_salvage;
          ...
          let dt_salvage;
          const dt_salvage_table = document.querySelector('.datatables-salvage');
          if (dt_salvage_table) {
            dt_salvage = new DataTable(dt_salvage_table, {
              searching: false,  // Opsi ini akan menghilangkan input cari
              ...
              columns: [
                ...
                { data: 'id', width: '20px' },
                { data: 'nama_sparepart' },
                { data: 'qty', width: '100px' },
                { data: 'cek', width: '100px' },
                { data: 'cek', width: '100px' },
              ],
              columnDefs: [
                {
                ...
                },
                {
                  ...
                  render: function (data, type, full, meta) {
                    return `
                      ${data} 
                      <input type="hidden" name="salvage[${full.id}][sparepart]" value="${full.kode_sparepart}" />
                    `; 
                  }
                },
                {
                  ...
                  render: function (data, type, full, meta) {
                    return `
                      ${data} 
                      <input type="hidden" name="salvage[${full.id}][qty]" value="${full.qty}" />
                    `; 
                  }
                },
                {
                  ...
                  render: function (data, type, full, meta) {
                    ...
                    return `<input type="checkbox" name="salvage[${full.id}][ada]" class="dt-checkboxes-det form-check-input chk-ada" value="1" ${chked}>`;
                  }
                },
                {
                  targets: 4,
                  ...
                  render: function (data, type, full, meta) {
                    ...
                    return `<input type="checkbox" name="salvage[${full.id}][tidak]" class="dt-checkboxes-det form-check-input chk-tidak" value="1" ${chked}>`;
                  }
                }
              ]
            });

            // Jika checkbox 'ADA' dicentang
            $('.datatables-salvage').on('change', '.chk-ada', function() {
              if ($(this).is(':checked')) {
                // Cari elemen 'TIDAK' di baris (tr) yang sama, lalu hilangkan centangnya
                $(this).closest('tr').find('.chk-tidak').prop('checked', false);
              }
            });

            // Jika checkbox 'TIDAK' dicentang
            $('.datatables-salvage').on('change', '.chk-tidak', function() {
              if ($(this).is(':checked')) {
                // Cari elemen 'ADA' di baris (tr) yang sama, lalu hilangkan centangnya
                $(this).closest('tr').find('.chk-ada').prop('checked', false);
              }
            });
          }
    ...
    const btnEditSelected2 = document.querySelector('.cetak-data');
    if (btnEditSelected2) {
        ...
        if (!selectedCheckbox) {
          ...
          .then(result => {
            ...
            if(status) {
              ...
              if(data.no_salvage) {
                const printUrl = `${baseUrl}administrasi/cetak-salvage?` + queryString;
                window.open(printUrl, '_blank');
              } else {
                // Swal.fire({
                //   icon: 'warning',
                //   title: `Peringatan!`,
                //   text: `Salvage belum dibuat!`,
                //   customClass: {
                //     confirmButton: 'btn btn-success'
                //   }
                // });
                ...
                Swal.fire({
                  title: 'Konfirmasi?',
                  text: "Salvage belum dibuat! Anda yakin akan mencetak data ini!",
                  icon: 'warning',
