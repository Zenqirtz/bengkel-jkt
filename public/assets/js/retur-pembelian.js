 ...
 */
 ...
 document.addEventListener('DOMContentLoaded', function (e) {
  ...
  let selected_no_retur  = '';
  let selected_tipe = 'detail';
  let rowToEdit = null;
  let dt_basic, dt_detail, dt_spk;
 
   // Variable declaration for table
   const dt_basic_table = document.querySelector('.datatables-retur'),
   ...
   if (dt_basic_table) {
    ...
    if (dt_detail_table) {
      dt_detail = new DataTable(dt_detail_table, {
        ...
        drawCallback: function() {
          ...
          $('.datatables-detail').off('change', '.dt-checkboxes-det').on('change', '.dt-checkboxes-det', function() {
    ...
    //           if (result.value) {
    //             // delete the data
    //             fetch(`${baseUrl}retur-pembelian-list/${user_id}`, {
    //               method: 'DELETE',
    //               headers: {
     ...
     document.addEventListener('click', function (e) {
       if (e.target.closest('.edit-record')) {
          if(isEdit) {
            ...
            // get data
            fetch(`${baseUrl}retur-pembelian-list/${user_id}/edit`)
    ...
    const btnEditSelected = document.querySelector('.edit-selected-retur');
    if (btnEditSelected) {
      btnEditSelected.addEventListener('click', function () {
        ...
        const selectedCheckbox = document.querySelector('.datatables-retur .dt-checkboxes:checked');
        ...
        if (!selectedCheckbox) {
          ...
          Swal.fire({
            icon: 'warning',
            title: 'Peringatan',
            text: 'Silahkan pilih (checklist) data Retur Gudang!',
            customClass: { confirmButton: 'btn btn-primary' }
          });
        } else {
          ...
          .then(result => {
            ...
            if(status) {
              ...
              document.getElementById('tipe').value = data.kode_tipe_barang;
              document.getElementById('user_id').value = data.id;
              document.getElementById('status_approve').value = '0';

              document.getElementById('add-nomor-input').value = data.kode_input;
              document.getElementById('add-nomor-input2').value = data.kode_input;
              ...
              document.getElementById('add-nomor-retur').value = data.no_retur;
              document.getElementById('add-tanggal-retur').value = data.tanggal;
              document.getElementById('add-nomor-bon').value = data.no_bon;
              document.getElementById('add-memo').value = data.memo;

              selected_no_retur = data.no_retur;
    ...
    const btnApproveSelected = document.querySelector('.approve-selected-retur');
    if (btnApproveSelected) {
      btnApproveSelected.addEventListener('click', function () {
        ...
        if (!selectedCheckbox) {
          ...
          // selected_no_retur = $("#add-nomor-order").val(); //data2.kode_order; // Sesuaikan dengan nama field di database/json
    ...
    const btnEditSelected2 = document.querySelector('.cetak-selected-retur');
    if (btnEditSelected2) {
          ...
          .then(result => {
            ...
            if(status) {
              ...
              if(data.status_approve == '1') {
                ...
                const printUrl = `${baseUrl}gudang/retur-pembelian-cetak?` + queryString;
                ...
                Swal.fire({
                  icon: 'warning',
                  title: `Peringatan!`,
                  text: `Data Retur Gudang masih menunggu approval`,
                  customClass: {
                    confirmButton: 'btn btn-success'
                  }
                });
      ...
      const fv = FormValidation.formValidation(addNewDataForm, {
        fields: {
          kode_input: {
            validators: {
              notEmpty: {
                message: 'Silahkan Input Nomor Input Gudang'
              }
            }
          },
          tanggal: {
        ...
        fetch(`${baseUrl}retur-pembelian-list`, {
          method: 'POST',
          headers: {
            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
    ...
    if (cekSPKBtn) {
      cekSPKBtn.addEventListener('click', function () {
        ...
        selected_tipe = 'input-gudang-all';
    ...
    if (btnSpkSelected) {
      btnSpkSelected.addEventListener('click', function () {
        ...
        if (!selectedCheckbox) {
          ...
          // Pastikan route URL sesuai dengan konfigurasi route Anda
          const url = `${baseUrl}gudang/retur-pembelian-cek-spk?` + queryString;
  ...
  function fetchDashboardOrder() {
    ...
    // Fetch data
    fetch(`${baseUrl}retur-pembelian-list?tipe=total-data`)
    .then(response => {
      if (!response.ok) throw new Error('Gagal mengambil data');
      return response.json();
    })
 ...
 function clearFormData() {
  ...
  //  fetchPermintaan('#add-tipe-barang', '#add-kode-permintaan');
   ...
   $('#add-nomor-input').addClass('is-invalid');  
   $('#add-tanggal-retur').addClass('is-invalid');  
  ...
  function hitungTotalPerbaikan() {
    ...
    $('#total-barang').text(formattedTotal);
