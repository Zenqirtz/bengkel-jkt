 ...
 */
 ...
 document.addEventListener('DOMContentLoaded', function (e) {
   ...
   const dt_basic_table = document.querySelector('.datatables-bank'),
  ...
  if (dt_basic_table) {
    ...
    const dt_basic = new DataTable(dt_basic_table, {
      ...
      ajax: {
        ...
        data: function (d) {
          ...
          d.no_rekening = $('#filter-nomor-rekening').val();
          d.nama_bank = $('#filter-nama-bank').val();
          d.lokasi_bank = $('#filter-lokasi-bank').val();
    ...
    document.addEventListener('click', function (e) {
      const chk = e.target.closest('.datatables-bank .dt-checkboxes');
      if (chk) {
        if (chk.checked) {
    ...
    if (btnEditSelected) {
      btnEditSelected.addEventListener('click', function () {
        ...
        const selectedCheckbox = document.querySelector('.datatables-bank .dt-checkboxes:checked');
        ...
        if (!selectedCheckbox) {
          ...
          // get data
          fetch(`${baseUrl}bank-list/${user_id}/edit`)
          ...
          .then(data => {
            ...
            document.getElementById('add-nomor-rekening').value = data.no_rekening;
            document.getElementById('add-nama-bank').value = data.nama_bank;
            document.getElementById('add-lokasi-bank').value = data.lokasi_bank;
      ...
      btnDeletedSelected.addEventListener('click', function () {
        ...
        if (!selectedCheckbox) {
          ...
          Swal.fire({
            ...
            if (result.value) {
              // delete the data
              fetch(`${baseUrl}bank-list/${user_id}`, {
                method: 'DELETE',
                headers: {
    ...
    if (addNewBtn) {
      addNewBtn.addEventListener('click', function () {
        if(isAdd) {
          ...
          document.getElementById('add-nama-bank')?.focus();
        } else {
          // Hide offcanvas
    ...
    const fv = FormValidation.formValidation(addNewDataForm, {
      fields: {
        nama_bank: {
          validators: {
            notEmpty: {
              message: 'Silahkan Input Nama Bank'
            },
            stringLength: {
              max: 50,
        ...
        no_rekening: {
          validators: {
            notEmpty: {
              message: 'Silahkan Input Nomor Rekening'
            }
          },
          stringLength: {
            max: 20,
 ...
 function clearFormData() {
    ...
    clearSelect('#add-kategori');
    clearSelect('#add-status-aktif');

    $('#add-nomor-rekening').addClass('is-invalid');
    $('#add-nama-bank').addClass('is-invalid');
    // $('#add-kategori').addClass('is-invalid');
