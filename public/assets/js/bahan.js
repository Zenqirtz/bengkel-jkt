 ...
 */
 ...
 document.addEventListener('DOMContentLoaded', function (e) {
   ...
   const dt_basic_table = document.querySelector('.datatables-bahan'),
  ...
  document.getElementById('add-harga')?.addEventListener('input', hitungHargaKonversi);
  document.getElementById('add-konversi')?.addEventListener('input', hitungHargaKonversi);
  ...
  if (dt_basic_table) {
    ...
    const dt_basic = new DataTable(dt_basic_table, {
      ...
      scrollX: true,
      ajax: {
        url: baseUrl + 'bahan-list',
        data: function (d) {
          // Ambil data dari input form modal dan masukkan ke parameter request
          d.nama_bahan = $('#filter-nama-bahan').val();
          d.kode_group_bahan = $('#filter-group-bahan').val();
          d.kode_satuan = $('#filter-satuan-beli').val();
          d.kode_satuan2 = $('#filter-satuan-pakai').val();
    ...
    document.addEventListener('click', function (e) {
      const chk = e.target.closest('.datatables-bahan .dt-checkboxes');
      if (chk) {
        if (chk.checked) {
    ...
    if (btnEditSelected) {
      btnEditSelected.addEventListener('click', function () {
        ...
        const selectedCheckbox = document.querySelector('.datatables-bahan .dt-checkboxes:checked');
        ...
        if (!selectedCheckbox) {
          ...
          // get data
          fetch(`${baseUrl}bahan-list/${user_id}/edit`)
          ...
          .then(data => {
            ...
            document.getElementById('add-nama-bahan').value = data.nama_bahan;
            // document.getElementById('add-harga').value = data.harga;
            // document.getElementById('add-konversi').value = data.konversi;
            // document.getElementById('add-harga-konversi').value = data.harga_konversi;

            setSelectValue('#add-group-bahan', data.kode_group_bahan, '');
            setSelectValue('#add-satuan-beli', data.kode_satuan, '');
            setSelectValue('#add-satuan-pakai', data.kode_satuan2, '');
      ...
      btnDeletedSelected.addEventListener('click', function () {
        ...
        if (!selectedCheckbox) {
          ...
          Swal.fire({
            ...
            if (result.value) {
              // delete the data
              fetch(`${baseUrl}bahan-list/${user_id}`, {
                method: 'DELETE',
                headers: {
    ...
    if (addNewBtn) {
      addNewBtn.addEventListener('click', function () {
        if(isAdd) {
          ...
          document.getElementById('add-nama-bahan')?.focus();
        } else {
          // Hide offcanvas
    ...
    const fv = FormValidation.formValidation(addNewDataForm, {
      fields: {
        ...
        kode_group_bahan: {
          validators: {
            notEmpty: {
              message: 'Silahkan Pilih Group Bahan'
            }
          }
        },
        is_active: {
  ...
  // Auto calculate Harga Konversi
  function hitungHargaKonversi() {
    // Ambil elemen input di baris tersebut
    let elHarga = $('#add-harga');
    let elKonversi = $('#add-konversi');
    let elHargaKon = $('#add-harga-konversi');

    // Ambil Nilai & Bersihkan format (hapus koma)
    let harga = parseFloat(elHarga.val().replace(/,/g, '')) || 0;
    let konversi = parseFloat(elKonversi.val().replace(/,/g, '')) || 0;

    // Hitung Subtotal Baris
    let subtotal = konversi > 0 ? (harga / konversi) : 0;
    ...
    elHargaKon.val(formattedSubtotal);
  }
 ...
 function clearFormData() {
    ...
    clearSelect('#add-satuan-beli');
    clearSelect('#add-satuan-pakai');
    clearSelect('#add-status-aktif');

    $('#add-nama-bahan').addClass('is-invalid');
    $('#add-group-bahan').addClass('is-invalid');
