 ...
 */
       ...
       if (e.target.closest('.delete-record')) {
          if (isDelete) {
            ...
            Swal.fire({
              ...
              if (result.value) {
                // delete the data
                fetch(`${baseUrl}spk-tutup-list/${user_id}`, {
                  method: 'DELETE',
                  headers: {
     ...
     document.addEventListener('click', function (e) {
       if (e.target.closest('.edit-record')) {
          if(isEdit) {
            ...
            // get data
            fetch(`${baseUrl}spk-tutup-list/${user_id}/edit`)
          ...
          if (!selectedCheckbox) {
            ...
            .then(result => {
              ...
              if(status) {
                ...
                document.getElementById('add-nomor-tutup').value = data.kode_tutup;
                document.getElementById('add-tgl-tutup').value = data.tanggal_tutup;
                document.getElementById('add-turun').value = data.kode_turun_lapangan;
                document.getElementById('add-tgl-turun').value = data.tgl_turun_lapangan;
      ...
      const FormValidation2 = FormValidation.formValidation(wizardPropertyListingFormStep2, {
        fields: {
          kode_tutup: {
            validators: {
              notEmpty: {
                message: 'Silahkan Input Nomor Tutup'
              }
            }
          },
          tanggal_tutup: {
            validators: {
              notEmpty: {
                message: 'Silahkan Input Tanggal Tutup'
              }
            }
          },
        },
 ...
 function clearFormData() {
  ...
  $('#add-nomor-tutup').addClass('is-invalid');
  $('#add-tgl-tutup').addClass('is-invalid');
