...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const dt_basic_table = document.querySelector('.datatables-ump');
  ...
  if (dt_basic_table) {
    ...
    const dt_basic = new DataTable(dt_basic_table, {
      ...
      scrollX: true,
      ajax: {
        url: baseUrl + 'uang-muka-penjualan-list',
        data: function (d) {
          d.no_transaksi = $('#filter-no-transaksi').val();
          d.nama = $('#filter-nama').val();
          d.jenis_penerimaan = $('#filter-jenis').val();
    ...
    document.addEventListener('click', function (e) {
      const chk = e.target.closest('.datatables-ump .dt-checkboxes');
      if (chk && chk.checked) {
        $('.datatables-ump .dt-checkboxes').not(chk).prop('checked', false);
      }
    });
    ...
    if (btnEdit) {
      btnEdit.addEventListener('click', function () {
        ...
        const chk = document.querySelector('.datatables-ump .dt-checkboxes:checked');
        if (!chk) {
          alertPilihData();
          return;
        }
        ...
        fetch(`${baseUrl}uang-muka-penjualan-list/${chk.value}/edit`)
          .then(r => r.json())
          .then(res => {
            if (!res.status) {
              Swal.fire({
            ...
            setSelectValue('#add-jenis', d.jenis_penerimaan, '');
    ...
    if (btnDelete) {
      btnDelete.addEventListener('click', function () {
        ...
        Swal.fire({
          ...
          if (result.value) {
            fetch(`${baseUrl}uang-muka-penjualan-list/${chk.value}`, {
              method: 'DELETE',
              headers: {
    ...
    // ===== TOMBOL CETAK =====
    const btnPrint = document.querySelector('.print-record');
    if (btnPrint) {
      btnPrint.addEventListener('click', function () {
        ...
        window.open(`${baseUrl}keuangan/cetak-uang-muka-penjualan?id=${chk.value}`, '_blank');
      });
    }
  ...
  if (addNewDataForm) {
    const fv = FormValidation.formValidation(addNewDataForm, {
      fields: {
        ...
        jenis_penerimaan: { validators: { notEmpty: { message: 'Jenis Penerimaan wajib dipilih' } } },
      ...
      fetch(`${baseUrl}uang-muka-penjualan-list`, {
        method: 'POST',
        headers: {
          'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
  ...
  $('#add-kode-bank').on('change', function () {
    ...
    const namaBank = $('#add-kode-bank option:selected').text().trim();
    filterRekeningByBank(kodeBank, namaBank, '');
  });
...
function filterRekeningByBank(kodeBank, namaBank, preselect) {
  ...
  fetch(`${baseUrl}uang-muka-penjualan-list?rekening=1&kode_bank=${kodeBank}`)
    .then(r => r.json())
    .then(data => {
      ...
      // Hapus else fallback — biarkan kosong jika tidak ada rekening
    ...
    .catch(() => {
      // Hapus fallback di sini juga — biarkan kosong
      initSelect2Rekening($sel);
    });
}
