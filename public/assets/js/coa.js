 ...
 */
 ...
 document.addEventListener('DOMContentLoaded', function (e) {
   ...
   const dt_basic_table = document.querySelector('.datatables-coa'),
  ...
  if (dt_basic_table) {
    ...
    const dt_basic = new DataTable(dt_basic_table, {
      ...
      scrollX: true,
      ajax: {
        url: baseUrl + 'coa-list',
        data: function (d) {
          // Ambil data dari input form modal dan masukkan ke parameter request
          d.acct_cd = $('#filter-nomor-akun').val();
          d.descs = $('#filter-nama-akun').val();
          d.active_status = $('#filter-status-aktif').val();
        },
      ...
      columnDefs: [
        ...
        //  },
        {
          targets: -1,
          render: function (data, type, full, meta) {
            const status = full['active_status'];
    ...
    document.addEventListener('click', function (e) {
      const chk = e.target.closest('.datatables-coa .dt-checkboxes');
      if (chk) {
        if (chk.checked) {
    ...
    if (btnEditSelected) {
      btnEditSelected.addEventListener('click', function () {
        ...
        const selectedCheckbox = document.querySelector('.datatables-coa .dt-checkboxes:checked');
        ...
        if (!selectedCheckbox) {
          ...
          // get data
          fetch(`${baseUrl}coa-list/${user_id}/edit`)
          ...
          .then(data => {
            ...
            document.getElementById('add-nomor-akun').value = data.acct_cd;
            document.getElementById('add-nama-akun').value = data.descs;
            document.getElementById('add-level').value = data.ilevel;
            document.getElementById('add-no-urut').value = data.seq_no;

            setSelectValue('#add-akun-tipe',   data.acct_type,  '');
            setSelectValue('#add-status-aktif',   data.active_status,  '');
          });
        }
      });
    }
      ...
      btnDeletedSelected.addEventListener('click', function () {
        ...
        if (!selectedCheckbox) {
          ...
          Swal.fire({
            ...
            if (result.value) {
              // delete the data
              fetch(`${baseUrl}coa-list/${user_id}`, {
                method: 'DELETE',
                headers: {
    ...
    if (addNewBtn) {
      addNewBtn.addEventListener('click', function () {
        if(isAdd) {
          ...
          document.getElementById('add-nama-coa')?.focus();
        } else {
          // Hide offcanvas
    ...
    const fv = FormValidation.formValidation(addNewDataForm, {
      fields: {
        acct_cd: {
          validators: {
            notEmpty: {
              message: 'Silahkan Input Nomor Akun'
            },
            stringLength: {
              max: 10,
        ...
        descs: {
          validators: {
            notEmpty: {
              message: 'Silahkan Input Nama Akun'
            }
          }
        },
        active_status: {
 ...
 function clearFormData() {
    ...
    // clearSelect('#add-jenis-coa');
    clearSelect('#add-status-aktif');
    clearSelect('#add-akun-tipe');

    $('#add-nomor-akun').addClass('is-invalid');
    $('#add-nama-akun').addClass('is-invalid');
