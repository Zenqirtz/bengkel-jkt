...
class LaporanGudangBahanExport implements FromCollection, WithHeadings, WithMapping, ShouldAutoSize, WithStyles, WithEvents
{
  protected $params;
  protected $cabang;
  protected $periode;
  ...
  {
    $query = DB::table('v_rpt_gudang_bahan')
  ...
  public function map($row): array
  {
    if ($row->is_subtotal) {
      ...
      $last = end($this->rowValuesList);
      return ['', '', '', '', '', '', '', '', '', 'Total', $last['K'], $last['L'], $last['M'], $last['N'], $last['O']];
    }
    ...
    if (isset($row->is_grandtotal) && $row->is_grandtotal) {
      ...
      $last = end($this->rowValuesList);
      return ['', '', '', '', '', '', '', '', '', 'Grand Total', $last['K'], $last['L'], $last['M'], $last['N'], $last['O']];
    }

    if ($row->is_repeat_header) {
      $this->rowValuesList[] = ['type' => 'header'];
      return ['No', 'Tanggal', 'No. Input Gudang', 'Supplier', 'Nama Bahan', 'Tipe', 'No. PO', 'Qty', 'Satuan', 'Harga', 'Jumlah', 'PPN', 'Total', 'Kas', 'Kredit'];
    }
