...
// class LaporanAgingPenagihanExport implements FromCollection, WithHeadings, WithMapping, ShouldAutoSize, WithStyles, WithEvents
class LaporanAgingPenagihanExport extends DefaultValueBinder implements FromCollection, WithHeadings, WithMapping, ShouldAutoSize, WithStyles, WithEvents, WithCustomValueBinder
{
    protected $params;
    protected $cabang;
    protected $periode;
    ...
    public function collection()
    {
        ...
        if($this->params['jenis_laporan'] == "rekap") {
          ...
          $this->grandTotals = [
            ...
            // --- HEADER DOKUMEN ---
            $exportData[] = ["Laporan Rincian Kwitansi Belum Ditagih [Outstanding Penagihan]"];
            ...
            foreach ($grouped as $asuransi => $asuransiGroup) {
                ...
                $totalPelangganOR = 0;
                $totalPelangganUangMuka = 0;
                $totalPelangganSisa = 0;
                ...
                foreach ($mingguGrouped as $minggu => $items) {
                    ...
                    $exportData[] = [
                        "No", "Tanggal Kwitansi", "No. Kwitansi", "No. SPK", "No. Polisi", 
                        "Merek Tipe", "No. Klaim", "No. Polis / No. Tiket", "Tertanggung",
                        "Total Tagihan", "Total OR", "Uang Muka", "Sisa Tagihan", 
                    ...
                    foreach ($items as $item) {
                        $sisaTagihan = $item->total - ($item->total_or_ass + $item->uang_muka);

                        $exportData[] = [
                            $no++,
                            $item->tgl_kwitansi ? date("d/m/Y", strtotime($item->tgl_kwitansi)) : '',
                            $item->kode_kwitansi,
                            $item->kode_spk,
                            $item->no_polisi,
                            $item->merek_tipe,
                        ...
                        $subTotalOR += (float) $item->total_or_ass;
                        $subTotalUangMuka += (float) $item->uang_muka;
                        $subTotalSisa += (float) $sisaTagihan;
                    }

                    // Cetak Baris Sub Total
                    $exportData[] = [
                        "", "", "", "", "", "", "", "Sub Total", $subTotalUnit . " Unit", $subTotalNominal, (string) $subTotalOR, (string) $subTotalUangMuka, $subTotalSisa, "", "", ""
                    ];
                    ...
                    $totalPelangganOR += $subTotalOR;
                    $totalPelangganUangMuka += $subTotalUangMuka;
                    $totalPelangganSisa += $subTotalSisa;
                }

                // Cetak Baris Total Per Pelanggan
                $exportData[] = [
                    "", "", "", "", "", "", "", "Total Per Pelanggan", $totalPelangganUnit . " Unit", $totalPelangganNominal, (string) $totalPelangganOR, (string) $totalPelangganUangMuka, $totalPelangganSisa, "", "", ""
                ];
    ...
    public function headings(): array
    {
        if($this->params['jenis_laporan'] == "rekap") {
            return [
                ['Laporan Rekap Kwitansi Belum Ditagih [Outstanding Penagihan]'],                                // Baris 1: Judul
            ...
            return [
                AfterSheet::class => function(AfterSheet $event) {
                    ...
                    foreach ($this->headerRows as $idx => $headerRow) {
                        ...
                        $sheet->getStyle("A{$headerRow}:P" . ($subTotalRow - 1))->applyFromArray([
                            'borders' => [
                                'allBorders' => ['borderStyle' => Border::BORDER_THIN]
                            ]
                        ]);
                    ...
                    foreach ($this->subTotalRows as $r) {
                        $sheet->getStyle("H{$r}:M{$r}")->getFont()->setBold(true);
                    ...
                    $sheet->getStyle("J1:M{$highestRow}")->getNumberFormat()->setFormatCode('#,##0;-#,##0;0');
